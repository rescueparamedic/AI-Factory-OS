...
from agent_runtime.agent_pipeline import AgentPipeline
...
class AIFactoryKernel:
    def __init__(self):
        ...
        self.agent_pipeline = AgentPipeline(self.base_path, self.planning_engine, self.real_ai_worker, self.worker_manager, self.event_bus)
    ...
    def boot(self):
        self.update_manager.version_manager.set_version("2.6.0", "v2.6-agent-session-message-bus", "mvp")
    ...
    def scheduler_enqueue(self, title: str, worker_id: str, payload: dict | None = None, priority: str = "medium", product_id: str = "blog_growth_analyzer", max_retries: int = 2): return self.task_scheduler.enqueue(title=title, worker_id=worker_id, payload=payload or {}, priority=priority, product_id=product_id, max_retries=max_retries)
    ...
    def run_agent_session_pipeline(self, request: str, product_id: str = "blog_growth_analyzer", provider_id: str = "mock"): return self.agent_pipeline.run(request=request, product_id=product_id, provider_id=provider_id)
    def get_latest_agent_session(self): return self.agent_pipeline.latest()
    def list_agent_sessions(self, limit: int = 10): return self.agent_pipeline.list_sessions(limit=limit)
